...
static func next_step(state: Dictionary, health: float = 1.0, thirst: float = 1.0, hunger: float = 1.0) -> String:
	...
	if as_dictionary(state.get("discovered_species", {})).is_empty():
		return "Deine erste Art · ziele auf ein Tier in der Nähe und beobachte es mit Linksklick. E zeigt die Inspektionsansicht."
	return "Weiter entdecken · beobachte andere Arten mit Linksklick. Neue Teile findest du im Editor mit F2."
