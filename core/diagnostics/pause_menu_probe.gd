...
func _tactical_pause(tribe: Node) -> void:
	...
	_expect(tree.paused and tribe.panel.owns_world_pause(), "Space did not enter tactical pause")
	await _key(KEY_ESCAPE)
	_expect(flow.pause_open and tree.paused, "Tactical pause blocked Esc")
	await _click(flow._overlay.find_child("PauseSettings", true, false))
	_expect(settings.is_menu_open(), "Tactical pause blocked settings")
	settings._graphics_settings.controls.exposure.grab_focus()
	await _key(KEY_SPACE)
	_expect(tree.paused and settings.is_menu_open(), "Space unpaused the world behind settings")
	await _key(KEY_ESCAPE)
	await _key(KEY_ESCAPE)
	_expect(tree.paused and not flow.pause_open and tribe.panel.owns_world_pause(), "Menu exit stole tactical pause")
	tree.root.gui_release_focus()
	await _key(KEY_SPACE)
	_expect(not tree.paused and tribe.is_active(), "Space did not resume after closing the menu")
	# The direct settings shortcut must preserve that same existing owner.
	await _key(KEY_SPACE)
	await _key(KEY_F8)
	settings._vsync_option.grab_focus()
	await _key(KEY_SPACE)
	_expect(tree.paused and settings.is_menu_open(), "F8 settings leaked Space into tactical pause")
	await _key(KEY_ESCAPE)
	_expect(tree.paused and not settings.is_menu_open() and tribe.panel.owns_world_pause(), "F8 settings discarded tactical pause")
	tree.root.gui_release_focus()
	await _key(KEY_SPACE)
	_expect(not tree.paused and tribe.is_active(), "F8 return did not restore tactical controls")
