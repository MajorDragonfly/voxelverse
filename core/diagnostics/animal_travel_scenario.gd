...
func run(tribe: Node, identity: String, carrier: String) -> Node:
	...
	# Loading is paused before any resident/animal resumes; economic state must
	# exactly match the far snapshot, including partial production and freight.
	for field: String in ["husbandry", "economy", "stock"]:
