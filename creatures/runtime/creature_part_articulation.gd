...
func play(action: String, duration: float = -1.0) -> bool:
	if action not in ["bite", "eat", "grip"]: return false
...
func is_active() -> bool:
	return not _action.is_empty() or (not _manual and (_mouth > 0.0 or _grip > 0.0))


func debug_state() -> Dictionary:
	return {"joints": _joints.size(), "action": _action, "time": _time,
		"mouth": _mouth, "grip": _grip, "manual": _manual}


func _apply() -> void:
	for joint: Dictionary in _joints:
		var amount: float = _mouth if joint.channel == "mouth" else _grip
