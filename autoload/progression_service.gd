...
func _ensure_part_model_unlocks(notify: bool = true) -> void:
	...
	for model: Dictionary in PartLibrary.MouthCatalog.get_parts() + PartLibrary.TailCatalog.get_parts() + PartLibrary.OrnamentCatalog.get_parts() + PartLibrary.WingCatalog.get_parts() + PartLibrary.FinCatalog.get_parts() + PartLibrary.EarCatalog.get_parts():
	...
	if notify:
		for id: String in added: part_unlocked.emit(id, "Model variant")
