...
Stand: 9. September 2026 · M1c-Katalog und Fernlandschaft: `f7040b1` · Zielmaßstab: reale Körpergrößen und Galaxie · M1b-Referenzbetrieb: `4b27e34` · vorheriger M1-Ausbau: `94b4dd3` · M1-Technikprototyp: `555f9efa2a16d8dde6fbf121157671ef944b2f83` · M0-Implementierung: `3f7f2252e3cbce868920dcf7c86beb3b47d17354` · visuelle Ausgangsbasis: `b1f1ef9c2c14a27269505da1d086091a0884563f`

Dieses Dokument bündelt das Zielbild, den tatsächlichen Stand und die Reihenfolge der nächsten Arbeiten. Es ist die zentrale Projektplanung. Die älteren Berichte unter `art/` dokumentieren einzelne Arbeitsstände; ihre Phasen A–F sind keine Spielphasen. M0 ist als Grundlagenpaket technisch geprüft. M1 ist als separates Planetenlabor implementiert und technisch geprüft; Nachweise und Integrationsgrenzen stehen im [M1-Bericht](docs/PLANET_M1.md), im [Unterwasser-/Voxel-Ausbau](docs/UNDERWATER_VOXEL_PLANETS.md) und im [M1b-Referenzbetrieb mit realen Größen](docs/REAL_SCALE_PLANETS.md). M1c ergänzt einen reproduzierbaren Galaxiekatalog mit dauerhaften Notizen; [Umsetzung und Abnahme](docs/GALAXY_CATALOG_AND_DISTANCE.md). Lars bewertet Berge und Wolken inzwischen positiv. Seine Rückmeldung zu unterbrochenen Fernbäumen und dem harten Übergang ins Inland wird in diesem Ausbau bearbeitet; erneuter Ziel-PC-Spieltest und Produktionsintegration bleiben offen. M0-Verträge: [Kampagne](docs/CAMPAIGN_CONTRACTS.md).
...
- Planeten sollen endliche, kugelförmige Himmelskörper in Originalgrößen entsprechend ihrer Körperklasse sein und eine zusammenhängende Oberfläche besitzen. Die 1–8-km-Laborkörper dienen schnellen Tests; der große Referenzkörper Terra hat 12.742 km Durchmesser. Die Oberfläche, die später aus dem Orbit sichtbar ist, gehört zur tatsächlich bespielten Welt.
...
| Galaxie | M1c-Katalog mit versionierten Sektor-/System-/Körperadressen, Ein-/Doppelsternen, realen Größen, begrenzten Caches und dauerhaft gespeicherten Namen/Notizen/Entdeckungsmarkierungen; Referenzdurchmesser 100.000 Lichtjahre | Zoomfähige Galaxiekarte, tatsächliches Systemstreaming, Reisen, Kolonien und Kampagnenanbindung in M9 | [M1c-Nachweise](docs/GALAXY_CATALOG_AND_DISTANCE.md), [GalaxyCatalog](world/space/galaxy_catalog.gd) |
| Sternsystem | Gemeinsame V9-Profilquelle; M1-Testsystem sowie M1b-Referenzen mit 100 / 1.000 / 12.742 km Durchmesser, astronomischen Bahnabständen, Ein-/Doppelstern und gemeinsamer Uhr | Produktive Kampagnenanbindung, weitere Konfigurationen und Inhalte | [PlanetCatalog](world/generation/planet_catalog_v7.gd), [CelestialSystem](world/space/celestial_system.gd) |
...
| Planetengeometrie | Bestand auf X/Z-Ebene; zusätzlich adaptive M1/M1b-Kugeln bis 12.742 km Durchmesser mit radialen Voxelstufen, gemeinsamen Nah-/Fernkanten, radialer Physik, präzisem Höhenfeld und Ozean | Produktive Landschaft/Flora/Fauna auf Kugel, Kampagnenmigration, kürzere Nachladepausen und zeitliche Glättung von Detailwechseln | [WorldGenerator](world/generation/world_generator_planetary_v9.gd), [AdaptiveSphereTiles](world/planet_lab/adaptive_sphere_tiles.gd) |
| Himmel und Wasseratmosphäre | Kamerabezogene Unterwassersicht und geschlossene Wasserunterseite; im M1-Labor Himmelskörper, Licht, Tageslauf und Systemkarte aus derselben Zeit-/Positionsquelle | Visuelle Atmosphärenausarbeitung und Übernahme in die Kampagne | [PlanetVisualEnvironment](world/visuals/planet_visual_environment.gd), [PlanetLab](world/planet_lab/planet_lab.gd) |
...
Die geplante Hierarchie lautet Universum → Galaxie → Sektor → Sternsystem → Himmelskörper → Oberflächenregion → Spielobjekt. M1c implementiert die Galaxie-/Sektor-/System-/Körperadressen und ihren Datenkatalog; M1/M1b prüfen die Körper-/Oberflächenebene. Die produktive Verbindung beider Ebenen mit Reisen und Kampagnenorten folgt später. Spezies und Fraktionen haben eigene Identitäten; eine Fraktion kann mehrere Regionen und Planeten besitzen, und eine Spezies kann mehrere Fraktionen bilden.
...
**In M1/M1b erprobt:** sechs verbundene Cube-Sphere-Flächen bis 12.742 km Körperdurchmesser; lokale Voxel, Physik, Wasser und Speicherung bleiben präzise. Der Referenzbetrieb erreicht auf Terra Detailstufe 19, höchstens 768 sichtbare Kacheln und 24 Kachelkollisionen. Noch vorhandene Ladepausen werden ausdrücklich angezeigt und gemessen; kontinuierliche Leistung auf dem Ziel-PC, weitere Körperklassen und Kampagnenintegration bleiben offen. [M1b-Nachweise und Grenzen](docs/REAL_SCALE_PLANETS.md).
...
**M0: technisch geprüft** (`3f7f2252e3cbce868920dcf7c86beb3b47d17354`, [Nachweise](docs/CAMPAIGN_CONTRACTS.md)). **M1: begrenzter Technikprototyp geprüft. M1b: Referenzbetrieb realer Größen technisch geprüft; Streamingoptimierung und Ziel-PC-Abnahme offen. M1c: Galaxiegrundlage und direkte Katalogbesuche mit gespeicherten Oberflächenorten umgesetzt; Ziel-PC-Spieltest, Flug und Kampagnenintegration offen. M2–M10: Planung bzw. parallele Arbeit auf eigenen Branches.** Lars' manueller Spieltest von M0 ist noch offen. Der vorhandene spielbare Stand oben bleibt die Ausgangsbasis für M1. Reihenfolge ist wichtiger als ein unbelegter Kalendertermin. Jeder Meilenstein endet mit einer kleinen prüfbaren Version.
...
| M1b | **Referenzbetrieb geprüft:** reale Planetengrößen | M1 | Neris 100 km, Orin 1.000 km und Terra 12.742 km; jeweils über 167 m echter Lauf, Flächenwechsel, lokale Voxel/Kollision, Pole, Wasser und präzise Orbit-/Save-Rückkehr. Speicher-/Kachelgrenzen bleiben begrenzt. Nachladeberechnung mit höchstens vier Workern, begrenztem Kachelcache, vorausschauender Auswahl und zeitlicher Detailüberblendung ausgebaut. Schnelle Läufe können weiter warten; Ziel-PC-Test offen. [Aktueller Ausbau und Messungen](docs/PLANET_STREAMING_AND_VISITS.md) |
| M1c | **Grundlage technisch geprüft:** Galaxieadressen und reproduzierbarer Systemkatalog | M1-Koordinatenvertrag; eigener Zweig über M1b | Versionierte IDs, 96 Sektoren/595 Systeme unabhängig von Besuchsreihenfolge und nach Prozessneustart geprüft; höchstens 16 Sektoren, 32 Systeme und 16 Änderungsdatensätze im Cache. Katalogansicht mit Namen, Notizen und Markierungen; Gesteinsplaneten/Monde direkt in Originalgröße besuchen. Katalog-ID, reales System und Körperort sind verbunden; Wiederbesuch über Sektoren und Prozessneustart geprüft. Galaxiekarte, Schiffsflug und Kampagnenmigration folgen in M9. [Aktueller Ausbau und Grenzen](docs/PLANET_STREAMING_AND_VISITS.md) |
...
**Historischer M1-Grundstand:** F4 öffnete das eigenständig gespeicherte Labor mit Haven (256 m Radius), Ember (160 m Radius) und Lune (64 m Radius), gemeinsamen V9-Profilen, sechs geschlossenen Kugelflächen, höchstens 24 Nahkacheln, radialer Bewegung und gemeinsamem Himmel/Orbit/System. Die bisherige Welt bleibt im Modus `legacy_plane_v9`. Koordinaten bei Erdradius sind rechnerisch geprüft; erdgroßes Terrain ist noch kein Laufzeitnachweis. Die konkrete M1-Entscheidung für M2 ist eine körperfeste Cube-Sphere-Adresse mit lokalem Ursprung; große Kugeln benötigen weitere Kachelunterteilung.

**M1-Ausbau – technisch geprüft (`6916cab`), eigener Branch `agent/planet-lod-menu`:** Aster ergänzt einen Planeten mit 4.096 m Radius, hierarchischer Unterteilung und Terrain-/Wasserübergängen. Die Hierarchie bleibt vollständig geschlossen, enthält höchstens 768 sichtbare Geländekacheln und 24 Kollisionskacheln. Ein Hintergrundauftrag berechnet die nächste Unterteilung; die alte Oberfläche bleibt bis zur vollständigen Veröffentlichung erhalten. Zusätzlich sind die fernen Voxelstufen der bisherigen Welt feiner; Esc/F8-Menü sowie F4-/Menüeinstieg bestehen die native Windows-/Linux-Eingabeprüfung. [Umsetzung und Nachweise](docs/PLANET_LOD_AND_MENU.md).

**Weitere Spieltest-Rückmeldung umgesetzt:** Lars bewertet das Planetensystem positiv; die Unterwasseransicht, bisherigen Miniaturdurchmesser und glatten Kugelböden brauchen Anpassung. Der separate Branch `agent/underwater-voxel-planets` ergänzt kamerabezogene Unterwasseratmosphäre, eine geschlossene Wasserunterseite, 0,5-m-Geländestufen und eine vorläufige Staffelung von 1,024 km Mond- bis 8,192 km Planetendurchmesser. Alle vier Körper verwenden adaptive Kacheln; alte Labor-Orte erhalten einmalig ihre passende Bodenfreiheit. Die neue Optik wurde in beiden Renderern aufgenommen und überprüft; der nächste Ziel-PC-Spieltest bleibt offen. [Aktueller Ausbau und Prüfstand](docs/UNDERWATER_VOXEL_PLANETS.md).
...
### M1b-Referenzbetrieb und M1c-Galaxiegrundlage

Die kleinen Kugeln bleiben Regressionstests. M1b betreibt drei Referenzkörper bis Erdgröße mit echten Voxelmeshes und Physik. Der aktuelle Ausbau verteilt die Kachelberechnung auf höchstens vier Worker, bereitet Nodes und nahe Kollisionsformen im Uploadbudget vor und hält bis zu 256 verdrängte Meshvarianten zur Wiederverwendung bereit. Alte und neue vollständige Geländeabdeckung wechseln über 180 ms; Land und Wasser teilen die Überblendungsphase. Der 167-m-Lauf mit 24 m/s, höchstens 4 m Zellenbreite, echten Flächenkanten und Kollisionsprüfungen bleibt die Messstrecke. Schnelle Läufe können weiterhin auf Nahgelände warten. [Messungen, Grenzen und aktuelles Testpaket](docs/PLANET_STREAMING_AND_VISITS.md).

**M1c ist als eigener Ausbau umgesetzt:** Der Katalog erzeugt Ein-/Doppelsternsysteme, Gesteinsplaneten, Gasriesen und Monde auf Anfrage. Versionierte Adressen behalten den 64-Bit-Seed und ganzzahlige Sektoren; lokale Versätze bleiben separat. Ein Labor-Menü erlaubt Abfragen weit entfernter Sektoren und dauerhaft gespeicherte Notizen. Cacheleerung oder ein neuer Prozess ändern die Ausgangssysteme nicht. Unbesuchte Systeme laden keine Voxel. Die Referenzausdehnung von 100.000 Lichtjahren legt weder die endgültige Sternzahl noch die Reisezeit fest. [M1c-Vertrag, Nachweise und Testpakete](docs/GALAXY_CATALOG_AND_DISTANCE.md).

**Fernlandschaft nach Lars’ Spieltest:** Mittlere und ferne Baum-LODs erhalten durchgehende Stämme und tragende Äste. Eine begrenzte Vorschau setzt dieselben Bäume jenseits geladener Chunks fort; das gemeinsame Geländematerial gleicht insbesondere Schnee-, Fels- und Grundfarben an. Die Vorschau benötigt höchstens 2.048 Bäume in sechs Rendergruppen, einen Worker und keine zusätzlichen Terrainkollisionen. Die Bildabnahme umfasst den erhöhten Blick ins Inland in zwei Welten und beiden Grafikmodi.

**Katalogbesuche sind jetzt angebunden:** Die Auswahl eines Gesteinsplaneten oder Mondes lädt dessen tatsächlichen Katalogkörper mit Radius, Seed, Schwerkraft und Sternsystem. Pro besuchtem Körper werden präziser Ort und Blickrichtung, pro System die Zeit gespeichert. Der letzte Ort öffnet nach einem Neustart erneut. Sterne und Gasriesen bleiben im System sichtbar und haben keinen Landezugang. Die bestehende Notizspeicherung und die Kampagnensicherung behalten ihre eigenen Verträge.

**Nächste Arbeit im Planeten-/Weltraumzweig:** Ziel-PC-Rückmeldung zu Katalogbesuchen, Detailüberblendung, Fernwald und Inlandübergang einarbeiten; die verbleibenden Nachladepausen und Hauptthreadspitzen beim schnellen Laufen gezielt weiter reduzieren. Zoomfähige Galaxiekarte, Schiffe, Kolonien und die vollständige Weltraumspielschleife bleiben M9. Parallele Kreaturen-, Sozial-, Menü- und Soundarbeit bleibt auf ihren eigenen Branches.
