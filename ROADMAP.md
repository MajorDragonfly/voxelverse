...
| Fortschritt | Arten-/Regionenentdeckung, Insight, Körperteilfreischaltungen; M2A-Verhaltenspunkte, sechs Knoten und Kauf-/Speicherregeln; in diesem separaten Branch Skilltree-Oberfläche und durchsuchbares Entdeckungsbuch | Reale Punktequellen und Anwendung der Boni im Spiel, getrennte Technikforschung, gesellschaftlicher Ruf | [ProgressionService](autoload/progression_service.gd), [Oberfläche und Nachweise](docs/PLAYER_PROGRESSION_UI.md) |
...
| M4 | Lebendige Tierwelt und Kreaturen-Spielablauf; Skilltree und reale Begegnungen als getrennte Teilpakete | M1–M3 | UI-Ausgangspaket PR #14. Folgebranch `agent/creature-behavior-gameplay`: Befreunden/Helfen/Jagd, persistente Beziehungen und vier wirksame Kreaturenboni implementiert und als Draft-PR #17 bereitgestellt. 52/52 Laufzeitprüfungen, grafische F/H/K-/Kaufabnahme und je 14/14 Windows-/Linux-Exportprüfungen bestanden. Manueller Spieltest offen. Entdeckungsbuch an eigenen Chat übergeben; Nestgruppe/Begleiterbefehle separat. Herde, Nahrungssuche und vollständiges Räuber-Beute-Spiel bleiben offen. [Umfang](docs/CREATURE_BEHAVIOR_GAMEPLAY.md), [spätere Phasen](docs/BEHAVIOR_FUTURE_PHASES.md) |
| M5 | Erster echter Wechsel Kreatur → Stamm | M4 | Nestgruppe bleibt eine Vorstufe der Kreaturenphase. Erst mit Gruppenaufgaben, Sammeln, Werkzeugherstellung, erster Behausung und Versorgung wird der echte Wechsel freigegeben. Dieselbe Spezies, Heimat und Mitglieder übernehmen; Vermächtnis, Besitz und Beziehungen bleiben nach Laden erhalten. [Bestätigter Ablauf und Abhängigkeiten](docs/DEVELOPMENT_PATH.md) |
