...
func _run() -> void:
	...
	await _key(KEY_W)
	await _key(KEY_Q)
	for key in [KEY_F2, KEY_F4, KEY_F7, KEY_F8, KEY_F10, KEY_P]:
		await _key(key)
	...
	_expect(not is_instance_valid(root.get_node("AudioManager")._panel), "F7 opened audio over the progression modal.")
