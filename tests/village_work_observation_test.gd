...
func _run() -> void:
	...
	if failures.is_empty(): print("VILLAGE_WORK_OBSERVATION_PASSED ", steps, " arrived steps compared with full copies")
	await finish()
...
func trial(label: String, f: Dictionary, index: int = 0, delta: float = 0.25, rate: float = 1.0) -> void:
	...
	var before: Dictionary = Work.snapshot(v, v.members[index])
...
func care_and_neighbor() -> void:
	...
	for action in ["pickup", "deliver", "pickup", "return"]:
		...
		var original: Dictionary = v.duplicate(true)
		var before: Dictionary = Work.snapshot(v, v.members[0])
...
func finish() -> void:
	print(JSON.stringify({"test": "village_work_observation", "passed": failures.is_empty(), "steps": steps, "failures": failures}))
