...
func _restart() -> void:
	...
	_expect(b.tribe.huts == 1 and b.tribe.project.is_empty() and b.tribe.members[0].cargo.is_empty(), "Held cargo did not resume exactly once: " + JSON.stringify({"worker": b.tribe.members[0], "project": b.tribe.project, "simulation": b.village_simulation, "position_key": Simulation.key(b.tribe.members[0].position), "target_key": Simulation.key(Work.target(b.tribe, b.tribe.members[0]))}))
...
func _route_encoding_checks(view: Dictionary) -> void:
	var simulation: Dictionary = view.village_simulation
	var target: Dictionary = view.tribe.project.entrance
	var parsed: Dictionary = Atomic.parse_dictionary(Atomic.stringify({"place": target})).place
	_expect(Simulation.key(target) == Simulation.key(parsed), "Precise route key changed across JSON.")
	var before: String = Atomic.stringify(simulation)
	var nearby: Dictionary = target.duplicate(true)
	nearby.height += 0.000001
	_expect(Simulation.road_to(simulation, nearby).is_empty(), "Uncertified nearby endpoint acquired a route.")
	_expect(Atomic.stringify(simulation) == before, "Route lookup mutated persisted roads.")
	# Leave the real restart fixture with historical keys and precise endpoints.
	# The following cold process must finish construction without recertification.
	var historical: Dictionary = {}
	for path: Array in simulation.roads.values():
		var old_key: String = JSON.stringify(JSON.parse_string(JSON.stringify(path.back()))).sha256_text()
		historical[old_key] = path.duplicate(true)
	simulation.roads = historical
	_expect(not Simulation.road_to(simulation, target).is_empty(), "Historical certified route was lost.")
	var legacy: Dictionary = JSON.parse_string(JSON.stringify(simulation))
	var legacy_target: Dictionary = JSON.parse_string(JSON.stringify(target))
	_expect(not Simulation.road_to(legacy, legacy_target).is_empty(), "Shortened legacy endpoint was lost.")
