...
func capacity_check() -> void:
	for duplicates: bool in [false, true]:
		...
		expect(population.animals.has("new-egg") and population.animals.size() == population.MAX_ANIMALS, "Bounded spawning lost the slot reserved for the fourth role")
		expect(population.last_spawn_attempts == 1, "Prioritized role exceeded the single-publication budget")
