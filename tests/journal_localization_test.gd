...
func _run() -> void:
	_check(Copy.text("Kopfmodule", "en") == "Head modules", "Integrated head category is translated")
	var integrated_parts := {"head_elephant_trunk": "Elephant trunk", "mouth_feline_snout": "Feline muzzle", "mouth_bear_snout": "Bear muzzle", "mouth_pig_snout": "Pig snout"}
	for id: String in integrated_parts:
		_check(Copy.part(id, "name", "en") == integrated_parts[id], "Integrated journal part name: " + id)
