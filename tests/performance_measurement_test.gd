...
func _initialize() -> void:
	...
	if OS.get_name() == "Linux" and FileAccess.file_exists("/proc/self/status"):
		var memory: Dictionary = Stats.process_memory()
		if memory.rss_bytes == null or memory.rss_bytes <= 0: failures.append("Self process RAM monitor returned no evidence.")
