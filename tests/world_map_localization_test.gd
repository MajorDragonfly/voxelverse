...
func _layouts() -> void:
	...
	var original_atlas: Dictionary = tracker.atlas.data.duplicate(true)
	var body: Dictionary = state.get_current_body_record()
	var selected_id := "atlas-locale-custom-76"
	for i in range(128):
		var title := "Weltkarte {name}" if i == 76 else "Eigenes Tal mit einem langen Ortsnamen %02d" % i
	...
	for language: String in ["en", "de", "en", "de"]:
		...
		_expect(map._place_offset == 64, "Language change reset the stored place page")
	...
	body.exploration_atlas = original_atlas
	tracker.invalidate()
	tracker.update_exploration(true)
	map._place_offset = 0
