...
func _run() -> void:
	...
	_expect(restored.completed_count() == Progress.STEPS.size() and restored.current_step().is_empty(), "Finished tutorial is still active.")
	# Supported v1 migration keeps partial work, but never reopens completed help.
	restored.import_state({"schema": 1, "skipped": false, "progress": {"look": 0.25, "move": 4, "jump": 1, "inspect": 1}})
	_expect(restored.current_step() == "look" and restored.done("move") and not restored.done("eat"), "Partial v1 guide migration lost progress.")
	restored.import_state({"schema": 1, "skipped": false, "progress": {"look": 0.55, "move": 4, "jump": 1, "inspect": 1, "tribe": 1}})
	_expect(restored.current_step().is_empty() and not restored.done("tribe"), "Finished v1 guide restarted or imported unknown milestones.")
	_expect(restored.select_chapter("home") and restored.current_step() == "home", "Opt-in chapter selection did not enable expanded help.")
	var chosen := Progress.new()
	chosen.import_state(restored.export_state())
	_expect(chosen.current_step() == "home" and not chosen.select_chapter("invalid"), "Chapter selection did not round-trip.")
	chosen.record("eat")
	_expect(chosen.done("eat") and chosen.current_step() == "home", "Out-of-order action changed chosen chapter.")
	chosen.record("tribe")
	_expect(chosen.current_step().is_empty() and not chosen.done("drink"), "Tribal handoff did not end optional exercises honestly.")
	...
	restored.import_state({"schema": 2.5, "skipped": false, "progress": {"jump": 1}})
	_expect(not restored.supported() and restored.export_state().schema == 2.5, "Fractional future version was downgraded.")
