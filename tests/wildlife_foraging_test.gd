...
func _interruptions_and_obstacles() -> void:
	...
	# Isolate the movement contract from the installed production component.
	var installed_social: Node = animal.get_node_or_null("SocialBehavior")
	if installed_social != null:
		animal.remove_child(installed_social)
		installed_social.queue_free()
