...
func _test_layered_chunk_scene() -> void:
	...
	if chunk == null:
		return
	_expect(chunk.get_node_or_null("FarTerrainMesh") != null, "Terrain chunk has no FarTerrainMesh.")
	_expect(chunk.get_node_or_null("ChunkLODControllerV7") != null, "Terrain chunk has no LOD controller.")
	_expect(chunk.get_node_or_null("MicroVoxelSurfaceV6") == null, "Floating terrain overlay is still active.")
	_expect(chunk.has_method("set_lod_tier"), "Terrain chunk cannot switch mesh LOD tiers.")
	_expect(chunk.get_script().resource_path.ends_with("terrain_chunk_v8.gd"), "Fast V8 terrain chunk is not active.")
	chunk.free()
...
func _test_far_mesh_job() -> void:
	...
	_expect(int(result.get("vertex_count", 0)) == columns * rows, "Far mesh job produced the wrong vertex count.")
	_expect(int(result.get("triangle_count", 0)) == 8, "Far mesh job produced the wrong triangle count.")
	var arrays_value: Variant = result.get("arrays")
	_expect(arrays_value is Array and arrays_value.size() == Mesh.ARRAY_MAX, "Far mesh job did not return complete mesh arrays.")
...
func _test_planet_catalog() -> void:
	...
	_expect(str(first.get("system_name", "")) == str(repeated.get("system_name", "")), "Same system seed produced a different system name.")
	var planet_count: int = PlanetCatalogV7.get_planet_count(first)
	_expect(planet_count >= 3 and planet_count <= 6, "Planet catalog count is outside the supported range.")
	...
	_expect(int(first_planet.get("planet_seed", 0)) == 424_242, "Planet one no longer represents the current world seed.")
	_expect(first_planet == repeated_planet, "Planet catalog is not deterministic.")
...
func _test_required_resources() -> void:
	for path in [
		...
		"res://world/visuals/terrain/terrain_chunk_visuals_v8.gd",
...
func _finish() -> void:
	if _failures.is_empty():
		print("Layered World V8 test passed.")
