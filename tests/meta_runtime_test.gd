...
func _test_clean_player_runtime() -> void:
	...
	_expect(player.has_method("get_interaction_target"), "Player has no robust target resolver.")
	_expect(player.has_method("get_nearby_wildlife"), "Player has no nearby-creature inspection API.")
	_expect(player.has_method("is_inspection_mode_enabled"), "Player has no inspection mode API.")
	...
	_expect(InputMap.has_action("inspection_mode"), "Creature inspection action is not registered.")
	var ray := player.get_node_or_null("CameraPivot/SpringArm3D/Camera3D/InteractionRay") as RayCast3D
	_expect(ray != null, "Player interaction ray is missing.")
	if ray != null:
		_expect((ray.collision_mask & 4) != 0, "Player targeting ray cannot see wildlife collision layer 4.")
	...
	if animator != null:
		...
		_expect(animator.has_method("trigger_bite"), "Adaptive locomotion has no bite animation API.")
	var target_hud := player.get_node_or_null("TargetHealthHUD")
	var inspection_hud := player.get_node_or_null("CreatureInspectionHUD")
	_expect(target_hud != null, "Combat target health HUD is not active.")
	_expect(inspection_hud != null, "Creature inspection HUD is not active.")
...
func _test_wildlife_combat(player: Node) -> void:
	...
	_expect(wildlife.has_method("get_target_health_data"), "Wildlife has no combat-health inspection API.")
	_expect(wildlife.has_method("get_inspection_data"), "Wildlife has no creature inspection stats API.")
	var maximum_health: float = float(wildlife.get("maximum_health"))
	var damage_probe: float = minf(maximum_health * 0.20, 10.0)
	wildlife.call("receive_creature_attack", damage_probe, player)
	_expect(float(wildlife.get("current_health")) < maximum_health, "Non-lethal bite did not reduce wildlife health.")
	var health_data: Dictionary = wildlife.call("get_target_health_data")
	_expect(float(health_data.get("health_ratio", 1.0)) < 1.0, "Target health data did not update after damage.")
	var inspection: Dictionary = wildlife.call("get_inspection_data")
	_expect(float(inspection.get("attack", -1.0)) >= 0.0, "Inspection data is missing attack stats.")
	_expect(int(inspection.get("part_count", -1)) >= 0, "Inspection data is missing modular part count.")
