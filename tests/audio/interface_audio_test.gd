...
func run() -> void:
	...
	var service: Node = root.get_node_or_null("ProgressionService")
	var fixture_service: bool = service == null
	if fixture_service:
		service = ScanProgressionFixture.new()
	...
	var saves: Node = root.get_node_or_null("SaveGameService")
	var fixture_saves: bool = saves == null
	if fixture_saves:
		saves = SaveFixture.new()
	...
	if fixture_saves:
		...
		check(audio._interface_sources.get("SaveGameService") == saves, "Real save service retains its single lifecycle binding")
