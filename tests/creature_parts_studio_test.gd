...
func _run() -> void:
	...
	if failures.is_empty():
		print("Creature parts studio passed: 46 part recipes, 11 end pieces, XYZ controls, undo, mirrored spikes, center sockets, 2/4/6-leg soles, terrain IK, wildlife floor alignment, cosmetic skins and save roundtrip.")
...
func _check_catalog_and_pairs() -> void:
	var count: int = 0
	for category in ["mouth", "head", "eyes", "legs", "arms", "tail", "horns", "plates", "spikes", "decor"]:
		for definition: Dictionary in Library.get_parts_for_category(category):
			...
			preview.free()
			count += 1
	# Six antler/crest families extend the 40 previously covered recipes. Every
	# recipe above must still produce real voxel geometry in the shared preview.
	_expect(count == 46 and Library.get_terminal_parts().size() == 11, "Part or end-piece catalog incomplete.")
	var blueprint: Dictionary = Assembly.create_default()
