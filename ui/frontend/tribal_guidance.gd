...
func bind(controller: Node) -> void:
	# Other observers may expose only the shared order signal (for example
	# audio fixtures). Tutorial binding requires the full guidance contract.
	if is_instance_valid(controller):
		for event in ["guidance_action", "order_resolved", "community_event"]:
			if not controller.has_signal(event):
				controller = null
				break
		if is_instance_valid(controller) and not controller.has_method("village"):
			controller = null
	...
	if is_instance_valid(tribe):
		...
		var previous_panel: Node = tribe.get("panel")
		if is_instance_valid(previous_panel): previous_panel.set_guidance("", "")
	tribe = controller
...
func refresh() -> void:
	if not is_instance_valid(tribe): return
	var panel: Node = tribe.get("panel")
	...
	panel.set_guidance(caption, detail)
