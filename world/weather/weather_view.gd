...
func _ready() -> void:
	...
	rain_material.albedo_color = Color("9bbac7")
...
func configure(seed_value: int) -> void:
	...
	_floors.fill(INF) # Do not rain through unprobed terrain/roofs.
...
func present(snapshot: Dictionary, underwater: bool, covered: bool) -> void:
	...
	if _rain.visible:
		...
		for i in range(count):
			...
			var point := Vector3(wrapf(drop.x + direction.x * clock * wind, -PATCH_RADIUS, PATCH_RADIUS),
				wrapf(drop.y * 40.0 - clock * 17.0, -20.0, 20.0),
				wrapf(drop.z + direction.z * clock * wind, -PATCH_RADIUS, PATCH_RADIUS))
			...
			_rain.multimesh.set_instance_transform(i, Transform3D(Basis.IDENTITY.scaled(Vector3.ZERO) if hidden else streak, point))
	_clouds.visible = clouds_enabled and not underwater
	...
	if _clouds.visible:
		...
		for i in range(CLOUD_COUNT):
			...
			point.x = wrapf(point.x + clock * direction.x * wind * 0.6, -720.0, 720.0)
			point.z = wrapf(point.z + clock * direction.z * wind * 0.6, -720.0, 720.0)
