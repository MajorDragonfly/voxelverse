...
static func key(place: Variant) -> String:
	# Normalize integer/float JSON types without rounding precise surface addresses.
	return JSON.stringify(JSON.parse_string(JSON.stringify(place, "", true, true)), "", true, true).sha256_text()

static func road_to(simulation: Dictionary, place: Variant) -> Array:
	var direct: Array = simulation.roads.get(key(place), [])
	if not direct.is_empty(): return direct
	# Existing saves use shortened-JSON hashes. Their certified endpoint is still
	# authoritative even when that hash changes after a precise JSON round trip.
	# Search at most MAX_ROADS paths; never infer a route from a nearby point.
	for path: Array in simulation.roads.values():
		if not path.is_empty() and path.back() == place: return path
	return []
...
static func path_to(data: Dictionary, simulation: Dictionary, from: Variant, to: Variant) -> Array:
	...
	var first: Array = road_to(simulation, from)
	var last: Array = road_to(simulation, to)
...
static func advance(body: Dictionary, clock: float, cooperation: float = 1.0, observer: Callable = Callable(), include_player: bool = false) -> bool:
	...
	for member: Dictionary in data.members:
		...
		var before: Dictionary = Work.snapshot(data, member) if observer.is_valid() else {}
		var effects: Array = []
	...
	for id: String in data.husbandry.records:
		...
		if H.pending(record) > 0 and not road_to(simulation, record.pickup).is_empty(): H.offer(data, id)
