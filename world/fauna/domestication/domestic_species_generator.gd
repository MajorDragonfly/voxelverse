...
static func create(body: Dictionary, group: String, seed_value: int) -> Dictionary:
	...
	Blueprint.set_body_part(blueprint, "body_balanced_core" if group == "companion" or eggs else "body_long_grazer")
	...
	Blueprint.set_body_shape(blueprint, (Vector3(1.1, 1.15, 1.5) if eggs else (Vector3(1.05, 0.8, 2.15) if group == "companion" else Vector3(1.55, 1.1, 2.8))) * variation)
	...
	for pair in range(1 if eggs else 2):
		var index: int = Blueprint.add_part(blueprint, "legs_walker" if group == "companion" or eggs else "legs_hoof")
		var part: Dictionary = blueprint["parts"][index]
		part["mirrored"] = true
		part["rotation"] = Vector3.ZERO
		part["scale"] = 0.85 if group == "companion" else 1.05
		part["end_part_id"] = "feet_pads" if group == "companion" or eggs else "feet_hooves"
	...
	for part: Dictionary in blueprint["parts"]:
		if part["category"] == "legs":
			part["anchor_t"] = 0.5 if eggs else (0.25 if pair_index == 0 else 0.75)
			pair_index += 1
	Anatomy.rebind_all_parts(blueprint)
	Assembly.normalize(blueprint)
	var suitability: Dictionary = Contract.suitability(group)
	var label: String = {"milk": "Milchweider", "work": "Lastläufer", "companion": "Spürläufer", "eggs": "Nestläufer"}[group]
	...
	return {"id": species_id, "body_id": body["id"], "species_seed": seed_value,
		...
		"visual_scale": 0.6 if eggs else (0.55 if group == "companion" else (0.88 if group == "work" else 0.74)),
