...
static func create_cell(seed_value: int, cell: Vector2i, profile: Dictionary) -> Dictionary:
	...
	if rng.randf() > 0.64:
		return {}
	var kinds: Array[String] = ["ridge", "mesa", "basin", "rock_spire", "ancient_grove", "coastal_teeth"]
	var kind: String = kinds[rng.randi_range(0, kinds.size() - 1)]
	var style: int = int(profile.get("terrain_style", 0))
	if style == 2 and rng.randf() < 0.40:
		kind = "mesa"
	elif style == 5 and rng.randf() < 0.50:
		kind = "ridge"
	var center := Vector2(cell) * CELL_SIZE + Vector2(rng.randf_range(60.0, 196.0), rng.randf_range(60.0, 196.0))
	return {"id": "%d:%d:%d" % [seed_value, cell.x, cell.y], "kind": kind,
		"center": center, "angle": rng.randf_range(0.0, TAU),
		"radius": rng.randf_range(48.0, MAX_RADIUS), "height": rng.randf_range(10.0, 22.0),
...
static func height_offset(landmark: Dictionary, point: Vector2) -> float:
	...
	match str(landmark["kind"]):
		"ridge":
			var spine: float = 1.0 - smoothstep(0.0, radius * 0.32, absf(delta.y + sin(delta.x / radius * 5.0) * radius * 0.06))
		...
		"basin":
			return -elevation * 0.32 * (1.0 - smoothstep(0.0, 0.85, distance))
		"rock_spire":
