...
## Reuse the actual mesh-array exporter, with one standard body per tail.
...
func _run() -> void:
	...
	for profile: Dictionary in Tails.get_parts():
		var design: Dictionary = Assembly.create_default()
		for index in range(design.parts.size() - 1, -1, -1):
			if design.parts[index].category == "tail": design.parts.remove_at(index)
		...
		Geometry.build(model, profile, {"category": "tail"}, design)
		var isolated: Array = []
		var body: Array = []
	...
	if file == null:
		push_error("Cannot write tail review.")
		quit(1)
		return
	file.store_string(JSON.stringify(result))
	file.close()
	print("TAIL_REVIEW_EXPORT_PASSED")
