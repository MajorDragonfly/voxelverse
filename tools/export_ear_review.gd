...
## Reuse the actual mesh-array exporter, with one standard body per ear.
...
func _run() -> void:
	...
	for profile: Dictionary in Ears.get_parts():
		var design: Dictionary = Assembly.create_default()
		for index in range(design.parts.size() - 1, -1, -1):
			if design.parts[index].category == "ears": design.parts.remove_at(index)
		...
		Geometry.build(model, profile, {"category": "ears"}, design)
		var isolated: Array = []
		var body: Array = []
		...
		preview._articulation.set_ear_pose(1.0)
		var stretched: Array = []
	...
	if file == null:
		push_error("Cannot write ear review.")
		quit(1)
		return
	file.store_string(JSON.stringify(result))
	file.close()
	print("EAR_REVIEW_EXPORT_PASSED")
