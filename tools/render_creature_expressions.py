...
def main():
    ...
    parser.add_argument('--eyes',action='store_true',help='Close-up review of four eye families and three lid poses')
    args=parser.parse_args()
    models=json.loads(args.source.read_text())
    fig=plt.figure(figsize=(14,13 if args.eyes else 9),facecolor='#0e2028')
    fig.suptitle('VOXELVERSE  /  Augen und Lider' if args.eyes else 'VOXELVERSE  /  Tieremotionen',color='#edf1df',fontsize=25,y=.96)
    fig.text(.5,.91,'Vier Augenformen · begrenzte Pupillen · vollständiger Lidschluss' if args.eyes else 'Gleiche Kreatur · sechs Reaktionen aus der Laufzeit',ha='center',color='#acc6c1',fontsize=13)
    ...
    for index,(faces,colors) in enumerate(geometry):
        ax=fig.add_subplot(4 if args.eyes else 2,3,index+1,projection='3d')
        ...
        if args.eyes:
            group=np.concatenate([g[0].reshape(-1,3) for g in geometry[(index//3)*3:(index//3+1)*3]])
            low=group.min(axis=0);high=group.max(axis=0)
        ax.set(xlim=(low[0]-.05,high[0]+.05),ylim=(low[1]-.05,high[1]+.05),zlim=(low[2]-.05,high[2]+.05))
        ax.set_box_aspect(high-low,zoom=1.22)
        ax.view_init(elev=9 if args.eyes else 16,azim=-78 if args.eyes else -52)
        ax.set_proj_type('ortho');ax.set_axis_off()
        ax.set_title(models[index]['name'],color='#edf1df',fontsize=14 if args.eyes else 18,pad=0)
