extends SceneTree
## Capture visually accepted revision-1 ears; never run implicitly in validation.
...
const Profiles = preload("res://creatures/catalog/creature_ear_catalog.gd")
...
func _initialize() -> void:
	...
	for id: String in ids:
		for shape: Vector3 in Checks.SHAPES:
			for side: float in [-1.0, 1.0]:
				...
				Geometry.build(node, Parts.get_part(id), {"category": "ears", "shape_scale": shape}, Blueprint.create_default())
	...
	file.close()
	print("EAR_BASELINE_CAPTURED: ", result.size())
	quit()
