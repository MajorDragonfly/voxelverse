...
func _run() -> void:
	...
	_expect(egg_floor_ticks >= 3 and egg.is_on_floor(), "Egg actor did not establish physical floor contact within 180 physics ticks")
	_expect(Catalog.BodyEvidence.approved(Catalog.species_for(body.fauna_catalog, egg.catalog_species.id)), "Egg actor has no approved body")
...
func _check_legacy_progression(before: Dictionary) -> void:
	...
	for id: String in ["wings_broad_feather", "wings_slender_feather", "wings_bat_membrane", "wings_long_insect"]:
		_expect(not old_unlocks.has(id), "Legacy fixture already contains new wing model: " + id)
		old_unlocks[id] = {"reason": "Model variant", "source_part": "decor_feathers", "order": old_unlocks.size()}
	for id: String in ["ears_cat_pointed", "ears_bear_round", "ears_rabbit_long", "ears_dog_floppy", "ears_elephant_broad", "ears_bat_large"]:
		_expect(not old_unlocks.has(id), "Legacy fixture already contains new ear model: " + id)
		old_unlocks[id] = {"reason": "Model variant", "source_part": "decor_feathers", "order": old_unlocks.size()}
	# Exact comparison still protects every historical unlock, discovery,
	# reward, research field and the sixteen specific additive model references.
...
func _populate() -> void:
	...
	# A wandering animal can be between step contacts at an arbitrary frame.
	# Require three actual consecutive grounded ticks; do not teleport, disable
	# movement or substitute a terrain query for the actor's physical contact.
	for tick in range(180):
		await tree.physics_frame
		var grounded: bool = false
		for actor: Node in tree.current_scene.population.animals.values():
			if actor.catalog_species.get("group") == "eggs": grounded = actor.is_on_floor()
		egg_floor_ticks = egg_floor_ticks + 1 if grounded else 0
		if egg_floor_ticks >= 3: return
