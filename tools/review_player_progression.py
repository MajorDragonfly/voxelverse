...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="voxelverse-progression-gui-") as temporary:
        ...
        process = subprocess.run([str(editor), "--path", str(args.project.resolve()),
                                  ...
                                  "--script", "res://tests/behavior_skill_tree_test.gd", "--", "--capture", str(output)],
    ...
    passed = process.returncode == 0 and not error and "PROGRESSION_UI_OK" in process.stdout and all(image["passed"] for image in images)
