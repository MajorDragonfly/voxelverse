...
def main():
    ...
    fig = plt.figure(figsize=(15, 8.5), facecolor="#081820")
    fig.suptitle("Voxelverse · Neue Tierfußformen", color="#edf1df", fontsize=24, y=0.98)
    fig.text(0.5, 0.915, "Godot-Voxelgeometrie · Standansicht und Sohle", ha="center", color="#a5b9b7", fontsize=12)
    light = np.array([0.3, 0.8, 0.5])
    light /= np.linalg.norm(light)
    for column, model in enumerate(models):
        for row, elevation in enumerate([24, -35]):
            ax = fig.add_subplot(2, 3, row * 3 + column + 1, projection="3d", computed_zorder=True)
            ...
            ax.view_init(elev=elevation, azim=-65)
            ax.set_axis_off()
            ax.set_title(model["name"] if row == 0 else "Sohlenansicht", color="#edf1df" if row == 0 else "#a5b9b7", fontsize=18 if row == 0 else 12, pad=0)
    fig.subplots_adjust(left=0.02, right=0.98, bottom=0.045, top=0.87, wspace=0, hspace=0)
