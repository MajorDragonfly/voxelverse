...
## Reuse the actual mesh-array exporter, with one standard body per wing.
...
func _run() -> void:
	...
	for profile: Dictionary in Wings.get_parts():
		var design: Dictionary = Assembly.create_default()
		for index in range(design.parts.size() - 1, -1, -1):
			if design.parts[index].category == "wings": design.parts.remove_at(index)
		...
		Geometry.build(model, profile, {"category": "wings"}, design)
		var isolated: Array = []
		var body: Array = []
		...
		preview._articulation.set_wing_pose(1.0)
		var stretched: Array = []
	...
	if file == null:
		push_error("Cannot write wing review.")
		quit(1)
		return
	file.store_string(JSON.stringify(result))
	file.close()
	print("WING_REVIEW_EXPORT_PASSED")
